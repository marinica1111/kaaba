...
public:
	typedef struct _GLOBAL {
		...
		bool dopium = false;
		bool bSendToEveryone = false;
		bool staticpos = false;

		int m_iWeaponIndex = -1;
		int m_iWeaponIndexSkins = -1;

		int m_nCurrentMinDmg = 100;
		int m_nLocalIndex = -1;

		__int64 m_nLocalSteamID = -2;

		bool m_bSendNextCommand = false;
		...
		std::vector<__int64> m_vecWhitelistedIds = {
			76561199099285145, // alpha (http://steamcommunity.com/profiles/76561199099285145)
			76561199006975792, // chase (http://steamcommunity.com/profiles/76561199006975792/)
			76561198881181853, // maxwell (https://steamcommunity.com/profiles/76561198881181853)
			76561198822012798, // elegant (https://steamcommunity.com/profiles/76561198822012798)
			76561198158624836, // fiske (https://steamcommunity.com/profiles/76561198158624836)
			76561198342222025, // fiske alt (https://steamcommunity.com/profiles/76561198342222025)
			76561198186159471, // destiny (https://steamcommunity.com/profiles/76561198186159471)
			7656119817821943, // yert
			76561199708869060, //nigga
			76561198830673437, //charming
			76561197972708983, //feels
	...
	config_option( Color_f, ascent, Color_f( 0.62352941176f, 0.79215686274f, 0.16470588235f, 1.0f ) );
	config_keybind( key );
	config_option( bool, watermark, true );
	config_option( float, size_x, 610.f );
	config_option( float, size_y, 415.f );
	config_option( bool, play_music_on_inject, true );
	config_option( bool, polish_mode, false );
	group_end( );
...
#pragma region Rage general group
	...
	config_option( bool, debug_logs, true );
	config_option( bool, fake_flick, false );
	config_option( float, fake_flick_angle, 180.f );
	config_option( int, fake_flick_factor, 6 );
	config_keybind( fake_flick_key );

	config_option( bool, fake_lag, false );
	config_keybind( fake_lag_key );

	config_option( bool, scale_damage_hp, false );

	// conditions
	config_option( bool, fake_lag_standing, false );
	config_option( bool, fake_lag_moving, false );
	config_option( bool, fake_lag_ladder, false );
	config_option( bool, fake_lag_unduck, false );
	config_option( bool, fake_lag_air, false );
	config_option( bool, fake_lag_weapon_activity, false );
	config_option( bool, fake_lag_accelerate, false );
	config_option( bool, fake_lag_shot, false );
	config_option( bool, fake_lag_reset_bhop, true );
	config_option( bool, fake_lag_fix_leg_movement, false );
	...
	config_option( bool, anti_aim_edge_dtc, false );
	config_option( bool, anti_aim_edge_dtc_freestanding_standing, false );
	config_option( bool, anti_aim_edge_dtc_freestanding_running, false );
	config_option( float, anti_aim_edge_dtc_freestanding_delay, 1.5f );
	config_option( int, anti_aim_edge_dtc_priority, 0 );
	...
	config_option( bool, anti_aim_fake_body_safe_break, false );
	config_option( int, anti_aim_fake_body_amount, 115 );

	config_option( bool, anti_aim_distortion, false );

	config_option( bool, anti_aim_distortion_force_turn, false );
	config_option( float, anti_aim_distortion_force_turn_factor, 34.f );
	config_option( float, anti_aim_distortion_force_turn_speed, 40.f );

	config_option( bool, anti_aim_distortion_shift, false );
	config_option( int, anti_aim_distortion_shift_await, 3 );
	config_option( float, anti_aim_distortion_shift_factor, 50.f );
	...
	config_option( Color_f, anti_aim_manual_arrows_color, Color_f( 1.0f, 1.0f, 1.0f, 0.6f ) );

	config_option( bool, anti_aim_manual_ignore_distortion, false );

	config_option( float, anti_aim_manual_left, 90.f );
	config_keybind( anti_aim_manual_left_key );

	config_option( float, anti_aim_manual_right, -90.f );
	config_keybind( anti_aim_manual_right_key );

	config_option( float, anti_aim_manual_back, 180.f );
	...
	config_option( bool, antiaim_resolver_plausible_edge, false );
	config_option( bool, antiaim_resolver_plausible_last_moving, false );
	config_option( bool, antiaim_resolver_snap_to_closest_yaw, false );
	...
	config_option( bool, ignore_limbs_move, false );
	config_option( bool, override_awp, false );

	config_option( int, minimal_damage, 30 );
	config_option( bool, minimal_damage_override, false );
	config_keybind( minimal_damage_override_key );
	config_option( int, minimal_damage_override_amt, 5 );
	...
	config_option( bool, quick_stop_stop_in_fire, false );
	config_option( bool, quick_stop_between_shots, false );
...
#pragma region ESP group
	...
	config_option( Color_f, draw_bomb_color, Color_f( 0.58823529411f, 0.78431372549f, 0.23529411764f, 1.0f ) );
	...
	config_option( Color_f, target_capsules_color, Color_f( 0.58823529411f, 0.78431372549f, 0.23529411764f, 1.0f ) );
	...
	config_option( bool, modulate_world, false );
	config_option( bool, modulate_props, false );
	config_option( Color_f, wall_color, Color_f( 1.0f, 1.0f, 1.0f, 1.0f ) );
	config_option( Color_f, props_color, Color_f( 1.0f, 1.0f, 1.0f, 1.0f ) );
	...
	config_option( Color_f, grenade_prediction_color, Color_f( 0.19607843137f, 0.58823529411f, 1.0f, 1.0f ) );
	
	config_option( bool, grenade_proximity_warning, false );
	config_option( Color_f, grenade_proximity_warning_color, Color_f( 0.19607843137f, 0.58823529411f, 1.0f, 1.0f ) );

	config_option( bool, grenades, false );
	config_option( Color_f, grenades_color, Color_f( 1.0f, 1.0f, 1.0f, 0.78431372549f ) );
	group_end( );
#pragma endregion
	...
	config_option( Color_f, skeleton_color, Color_f( 1.0f, 1.f, 0.69411764705f ) );
	...
	config_option( bool, latency, false );
...
#pragma region MISC group
	...
	config_option( bool, autostrafer_circle, false );
	config_keybind( autostrafer_circle_key );
	...
	config_option( bool, low_fps_warning, false );
	...
	config_option( int, ping_spike_amount, 150 );
	config_keybind( ping_spike_key );

	config_option( bool, lag_walk, false );
	config_keybind( lag_walk_key );
