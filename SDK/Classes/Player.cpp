...
CUserCmd &C_BasePlayer::m_PlayerCommand( ) {
	return *( CUserCmd * )( ( uintptr_t )this + 0x326C );
}
...
int C_CSPlayer::SetNextThink( int tick ) {
	...
	// note - maxwell; crash fix i guess? this randomly becomes null. lol.
	if( !next_think )
		return -1;
...
void C_CSPlayer::PostEntityThink( ) {
	...
	if( this && !this->IsDead( ) ) {
		...
		Memory::VCall<UpdateCollisionBoundsFn>( this, 329 )( this );

		if( this->m_fFlags( ) & FL_ONGROUND )
			*reinterpret_cast< uintptr_t * >( uintptr_t( this ) + 0x3004 ) = 0;

		if( *reinterpret_cast< int * >( uintptr_t( this ) + 0x28AC ) == -1 ) {
			using SetSequenceFn = void( __thiscall * )( void *, int );
			Memory::VCall<SetSequenceFn>( this, 213 )( this, 0 );
		}

		using StudioFrameAdvanceFn = void( __thiscall * )( void * );
		Memory::VCall<StudioFrameAdvanceFn>( this, 214 )( this );
...
bool C_CSPlayer::IsLocalPlayer( ) {
	// note - maxwell; find a better way to do this eventually?...
	// C_CSPlayer* pLocal = C_CSPlayer::GetLocalPlayer();

	return this->EntIndex( ) == /*pLocal->EntIndex()*/g_pEngine->GetLocalPlayer( );
}
...
Vector C_CSPlayer::GetEyePosition( bool ignore_modify_eye_pos ) {
	Vector eyePos = GetAbsOrigin( ) + m_vecViewOffset( );
...
Vector C_CSPlayer::GetCenterHitboxPos( int nHitbox ) {
	auto hdr = *( studiohdr_t ** )( this->m_pStudioHdr( ) );
	if( !hdr ) {
		return { };
	}

	auto hitboxSet = hdr->pHitboxSet( this->m_nHitboxSet( ) );
	if( !hitboxSet ) {
		return { };
	}

	auto pHitbox = hitboxSet->pHitbox( nHitbox );
	if( !pHitbox ) {
		return { };
	}

	auto vecHitboxPos = ( pHitbox->bbmax + pHitbox->bbmin ) * 0.5f;
	return vecHitboxPos.Transform( m_CachedBoneData( ).Base( )[ pHitbox->bone ] );
}
...
bool C_CSPlayer::SetupBones( matrix3x4_t *pBoneToWorld, int nMaxBones, int boneMask, float currentTime ) {
	...
	const auto framecount = g_pGlobalVars->framecount;
	const auto curtime_backup = g_pGlobalVars->curtime;
	const auto realtime_backup = g_pGlobalVars->realtime;
	const auto frametime = g_pGlobalVars->frametime;
	const auto absoluteframetime = g_pGlobalVars->absoluteframetime;
	const auto tickcount = g_pGlobalVars->tickcount;
	const auto interpolation_amount = g_pGlobalVars->interpolation_amount;

	float time = m_flSimulationTime( );
	int ticks = TIME_TO_TICKS( time );

	g_pGlobalVars->curtime = time;
	g_pGlobalVars->realtime = time;
	g_pGlobalVars->frametime = g_pGlobalVars->interval_per_tick;
	g_pGlobalVars->absoluteframetime = g_pGlobalVars->interval_per_tick;
	g_pGlobalVars->tickcount = ticks;
	...
	if( EntIndex( ) != g_pEngine->GetLocalPlayer( ) )
		m_fEffects( ) |= 8u;
	m_bIsJiggleBonesEnabled( ) = false;

	g_pGlobalVars->framecount = -999;
	...
	*boneSnapshot2 = bk_snapshot2;
	...
	g_pGlobalVars->curtime = curtime_backup;
	g_pGlobalVars->realtime = realtime_backup;
