...
void AlternativeAttack::RunAttacks( bool* sendPacket, Encrypted_t<CUserCmd> pCmd ) {
	if( !g_pEngine->IsInGame( ) || !g_Vars.rage.enabled )
		return;
	...
	if( m_AlternativeAttackData.m_pLocalPlayer->m_flNextAttack( ) > g_pGlobalVars->curtime || m_AlternativeAttackData.m_pLocalWeapon->m_flNextPrimaryAttack( ) > g_pGlobalVars->curtime )
		return;

	m_AlternativeAttackData.m_vecEyePos = m_AlternativeAttackData.m_pLocalPlayer->GetEyePosition( );
	for( int i = 1; i <= g_pGlobalVars->maxClients; i++ ) {
		C_CSPlayer* Target = ( C_CSPlayer* )g_pEntityList->GetClientEntity( i );
		if( !Target
			|| Target->IsDead( )
			|| Target->IsDormant( )
			|| !Target->IsPlayer( )
		...
		for( auto& record : lag_data->m_Records ) {
			if( g_LagCompensation.IsRecordOutOfBounds( record ) /*|| record.m_RecordFlags & ELagRecordFlags::RF_IsInvalid*/ ) {
				continue;
			}
	...
	if( hdr ) {
		...
		if( hitboxSet ) {
			...
			if( pStomach ) {
				...
				//printf("%i\n",fireData.m_iHitgroup);
				if( flDamage >= 115.f && bIsAccurate ) {
...
int AlternativeAttack::GetMinimalHp( ) {
	if( g_pGlobalVars->curtime > ( m_AlternativeAttackData.m_pLocalWeapon->m_flNextPrimaryAttack( ) + 0.4f ) )
		return 34;
