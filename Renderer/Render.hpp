...
namespace Render {
	namespace DirectX {
		...
		inline LPDIRECT3DVERTEXBUFFER9 m_vb;
		inline LPDIRECT3DINDEXBUFFER9 m_ib;
		inline IDirect3DStateBlock9 *m_stateblock;

		inline LPD3DXLINE m_line;
		inline LPD3DXSPRITE m_sprite;

		inline bool m_candraw = false;
	...
	namespace Engine {
		...
		void Arc( Vector2D pos, float radius, float start_angle, float percent, float thickness, Color color );
	}
